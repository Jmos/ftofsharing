...
void Mail::buildSendData(Format f)
{
	...
	else
	{
		...
		if(getPartCount()>1 && header().value("boundary")==""){
			d->header.insert("boundary","asdfghjklqwertzuiop");
			
			while(!testBoundary(header().value("boundary"))){
				d->header.insert("boundary",header().value("boundary")+"x");
			};
		}
