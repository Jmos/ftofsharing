...
public:
	...
	LHPop3Private(LHPop3* _parent, const QString &pop3, const QString & _user, const QString & _passwd):
	parent(_parent),
	pop3_server(pop3),
	socket(0),
	t(0),
...
LHPop3::LHPop3(const QString &smtp_server, const QString &user, const QString &passwd)
{
	d = new LHPop3Private(this, smtp_server, user, passwd);
...
void LHPop3::getAllMails(bool del)
{
        qDebug ("*** %s,%d : %s", __FILE__, __LINE__, "LHPop3::getAllMails");
	d->delete_mails = del;
	openConnection();
}

void LHPop3::openConnection()
{
...
void LHPop3::dnsLookupHelper(const QHostInfo &hostInfo)
{
	QString host = "";
	*d->mxLookup = hostInfo;
...
/*
void LHPop3::emitBytesWritten(int bw)
{
	d->bytes_written += bw;
...
void LHPop3::parseMail(const QString& mail_data)
{
	LHMail* mail = new LHMail();

        const char *debugPtr = mail_data.toAscii().constData();
        
	mail->setData(mail_data);
	emit newMail(mail);
}

void LHPop3::parseStatLine(const QString& line)
{
...
void LHPop3::readyRead()
{
	...
	if (d->wait_for_line)
	{
		...
		qDebug("ready Read, bytes: %d, nl: %d, r: %d", bytes, nl, r);
		if (nl>0)
		{
...
void LHPop3::nextLine(const QString& line)
{
	qDebug("nextLine: %s", line.toAscii());
...
QString LHPop3::getPop3Server () const
{
...
QString LHPop3::getPassword () const
{
        return d->passwd;
}
